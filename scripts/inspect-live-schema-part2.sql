-- ==============================================================================
-- سكربت معاينة - الجزء الثاني (للقراءة فقط، لا يعدّل شيئاً)
-- يكمل ما انقطع من الجزء الأول:
--   أعمدة الجداول المتبقية | السياسات | قيود CHECK | المفاتيح الأجنبية | الدوال | المشغلات
-- نفّذه في SQL Editor ثم انسخ قيمة الخلية (schema_dump_part2) والصقها في المحادثة
-- أو استخدم زر Export واختر JSON
-- ==============================================================================
...
select json_build_object(
  'columns_remaining', (
    ...
    where c.table_schema = 'public'
      and c.table_name in (
        'products','profiles','rewards_history','top_banners',
        'trusted_buyers','wallet_transactions','wallets'
      )
  ),
  'policies', (
    select json_agg(json_build_object(
